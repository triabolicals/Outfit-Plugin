use engage::{
    ...
    unity_engine::{IRectTransformMethods, Transform}
};
...
impl<T> Randomizer<T> for Vec<T> {
    ...
    fn get_remove_filter(&mut self, rng: Random_2, filter: impl Fn(&T) -> bool ) -> Option<T> {
        let list: Vec<usize> = self.iter().enumerate()
            ...
            .map(|(index, _element)| index).collect();
...
pub fn left_right_enclose(string: &String) -> Il2CppString {
    format!("{}{}{}",
...
pub fn get_default_asset_conditions() -> unity::Array::<Il2CppString> {
    let array = unity::Array::new(Il2CppString::class().raw(), 1).unwrap();
    array.set(0, "".into());
    array
}
...
pub fn hash_string<'a>(str: impl Into<Il2CppString>) -> i32 {
    let str = str.into();
    str.get_hash_code()
}
...
pub fn try_get_il2cpp_hash(str: Il2CppString) -> Option<i32> {
